...
def main(search_keyword, is_option: bool = True, page_limit: int=1, hidden_chrome: bool=False,):
    ...
    # with open('kw.csv', 'r', encoding='utf-8_sig') as f:
        # kw = f.read()
    ...
    while page <= page_limit:
        ...
        for recruit_elm in recruit_elms:
            ...
            try:
                ...
                count+=1
            if fail != True:
                eel.view_log_js(f"[成功]{count} 件目 (page: {page}) : {name}")
            else:
                eel.view_log_js(f"[失敗]{count} 件目 (page: {page}): {name}")
